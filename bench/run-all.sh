#!/usr/bin/env bash
# اجرای کل ماتریس بنچمارک و ذخیره در bench/results
set -u
cd "$(dirname "$0")/.."
...
run() {
  ...
  local out="bench/results/${scenario}-${c}.json"
  echo "=== $scenario concurrency=$c duration=${D}s ==="
  node --no-warnings bench/load.mjs "$scenario" "$c" "$D" > "$out" 2>"bench/results/${scenario}-${c}.err"
